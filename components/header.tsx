		...
		const handleScroll = () => {
			...
			if (header) {
				if (window.scrollY > 50) {
					header.classList.add("bg-background", "shadow", "backdrop-blur-2xl");
					header.classList.remove("lg:bg-transparent");
				} else {
					header.classList.add("lg:bg-transparent");
					header.classList.remove("bg-background", "shadow", "backdrop-blur-2xl");
				}
			}
		};
					...
					<div className="flex items-center">
						<Link className="flex items-center" href="/">
							<Image
								...
								priority
							/>
							<span className="text-lg md:text-xl font-bold text-primary font-gambarino">KRYS-MALE</span>
						</Link>
					</div>
						...
						<SheetContent className="px-4">
							<SheetHeader>
								...
								<SheetDescription className="sr-only">
									Navigate to the menu by tapping the icon. Click on the menu items to navigate to the
									respective pages.
								</SheetDescription>
							</SheetHeader>
							<ul className="flex flex-col *:not-last:*:border-b *:not-last:*:border-secondary">
								{links.map((link) => (
									<li className="basis-14" key={link.href}>
										<Link
											href={link.href}
											className="text-primary font-medium size-full flex items-center"
											key={link.href}
										>
											{link.label}
										</Link>
